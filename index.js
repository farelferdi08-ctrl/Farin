...
async function checkMaintenanceCron() {
    ...
    if (currentTime === "22:55" && !maintenanceNotifiedStart) {
        const msg = "🛠️ *MAINTENANCE HARIAN DIMULAI*\n\nSistem Farin Shop sedang offline otomatis (22:55 - 00:15 WIB).\nTransaksi OTP & Deposit dihentikan sementara untuk pemeliharaan rutin.";
        
        await bot.sendPhoto(config.idchannel, config.maintance, { caption: msg, parse_mode: "Markdown" });
    ...
    if (currentTime === "00:15" && !maintenanceNotifiedEnd) {
        const msg = "✅ *MAINTENANCE SELESAI*\n\nSistem Farin Shop sudah aktif kembali secara normal.\nSilakan lakukan transaksi OTP dan Deposit sekarang! 🚀";
        
        await bot.sendPhoto(config.idchannel, config.maintanceoff, { caption: msg, parse_mode: "Markdown" });
...
*Silakan pilih menu di bawah untuk memulai:*`;
    ...
    const options = {
        ...
        reply_markup: {
            inline_keyboard: [
          [
            { text: "📱 Order OTP FarinShop", callback_data: "choose_service" }
          ],
          [
...
`;
        ...
        // Logging jika terjadi error seperti bot bukan admin di channel atau link gambar mati
        console.log(`❌ Gagal kirim log foto ke channel ${CHANNEL_ID}:`, errChannel.message);
      }
    }
} catch (err) { // <--- TAMBAHKAN BARIS INI (CATCH UTAMA)
    console.error("❌ Error proses deposit success:", err);
  } // <--- TAMBAHKAN BARIS INI (PENUTUP TRY UTAMA)
} // <---
