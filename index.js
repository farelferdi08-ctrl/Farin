...
setInterval(checkMaintenanceCron, 5000);
...
async function guardAll(x) {
  ...
  let isPrivate = isCallback ? x.message.chat.type === "private" : x.chat.type === "private";
  
  const answer = (text, alert = true) => {
    if (isCallback) return bot.answerCallbackQuery(x.id, { text, show_alert: alert });
    return bot.sendMessage(chatId, text, { parse_mode: "Markdown" });
  };

// === ⚙️ CEK WAJIB JOIN CHANNEL (2 CHANNEL - PREMIUM STYLE) ===
  if (checkJoinChannel() && isPrivate && !isOwner) {
    try {
      // Ambil username dari URL channel 1 dan 2 secara otomatis
      const ch1 = config.urlchannel.replace("https://t.me/", "").replace("@", "");
      const ch2 = config.urlchannel2.replace("https://t.me/", "").replace("@", "");

      // Cek status member di channel 1
      const member1 = await bot.getChatMember(`@${ch1}`, userId);
      const isJoined1 = ["member", "administrator", "creator"].includes(member1.status);

      // Cek status member di channel 2
      const member2 = await bot.getChatMember(`@${ch2}`, userId);
      const isJoined2 = ["member", "administrator", "creator"].includes(member2.status);

      // Jika salah satu belum join, kirim pesan verifikasi
      if (!isJoined1 || !isJoined2) {
        if (!isCallback) {
          // 🛡️ ANTI DOUBLE SEND LOGIC (Tetap dipertahankan agar tidak spam)
          if (!global.lastJoinPrompt) global.lastJoinPrompt = {};
          const now = Date.now();
          if (global.lastJoinPrompt[userId] && (now - global.lastJoinPrompt[userId] < 3000)) {
            return true; 
          }
          global.lastJoinPrompt[userId] = now;

          // KIRIM PESAN DENGAN DESAIN KOMPLEKS & ELEGAN
          await bot.sendMessage(chatId, `
<b>🔐 VERIFIKASI KEANGGOTAAN</b>
<b>━━━━━━━━━━━━━━━━━━━━━━━</b>
<blockquote>👤 <b>PENGGUNA:</b> ${userName}
🆔 <b>USER ID:</b> \`${userId}\`

⚠️ <b>AKSES TERBATAS!</b>
Sistem mendeteksi Anda belum bergabung ke saluran resmi kami. Silakan selesaikan verifikasi di bawah untuk membuka akses fitur.

🔰 <b>LANGKAH-LANGKAH:</b>
1️⃣ Bergabung ke <b>Channel Testimoni</b>
2️⃣ Bergabung ke <b>Channel Informasi</b>
3️⃣ Klik tombol <b>Verifikasi</b> di bawah</blockquote>
<b>──────────────────────────</b>
<blockquote><b>Farin Shop — Security System</b>
<b>──────────────────────────</b>`,
            {
              parse_mode: "HTML",
              reply_markup: {
                inline_keyboard: [
                  ...
                  [{ text: "📢 CALLBACK HARGA", url: config.urlchannel2 }],
          ...
          await answer("❌ Kamu belum join kedua channel tersebut.", true);
        }
        return true; // Memblokir akses fitur selanjutnya
      }
    } catch (e) {
  ...
  try {
    ...
    if (isBlacklisted && !isOwner) {
      await answer(`🚫 *Akses Ditolak!*\n\nKamu telah diblacklist.\n\n📋 *Alasan:* ${isBlacklisted.alasan}`, true);
      return true;
    }
  } catch (err) {}

  // === ⚙️ Maintenance & Mode Lain (Tetap Ada) ===
  if (checkMaintenance() && !isOwner) { await answer("🔧 Bot sedang *maintenance*.", true); return true; }
  if (checkGroupOnly() && isPrivate && !isOwner) { await answer("👥 Bot hanya untuk *grup*.", true); return true; }
...
// ============================
  ...
  try {
    // Pengecekan ganda di kedua channel
    const member1 = await bot.getChatMember(`@${ch1}`, userId);
    const member2 = await bot.getChatMember(`@${ch2}`, userId);
    
    const isJoined = ["member", "administrator", "creator"].includes(member1.status) && 
                     ["member", "administrator", "creator"].includes(member2.status);

    if (isJoined) {
      // Hapus pesan peringatan akses ditolak
      await bot.deleteMessage(chatId, messageId).catch(() => {});

      await bot.answerCallbackQuery(query.id, { text: "✅ Terima kasih sudah bergabung!", show_alert: false });
      await bot.sendMessage(chatId, "🎉 Kamu sekarang sudah bisa menggunakan bot Farin Shop.");
    } else {
      await bot.answerCallbackQuery(query.id, { text: "🚫 Kamu belum join di KEDUA channel!", show_alert: true });
    }
  } catch (e) {
...
// Gabungan System Log, Auto Save ID, dan Guard
bot.on("message", async (msg) => {
  ...
  // 1. 🔥 CEK MAINTENANCE HARIAN (Bypass Owner)
  if (isMaintenanceActive() && !isOwner) return;

  // 2. ⚡ AUTO SAVE ID (Hanya dijalankan jika user belum ada di database)
  ...
  if (!users.includes(userId)) {
    ...
    const waktuSave = moment().tz("Asia/Jakarta").format("DD-MM-YYYY HH:mm:ss");
...
`, { parse_mode: "Markdown" }).catch(() => {});
  ...
  // 3. 🛡️ JALANKAN GUARD (Hanya jika pesan adalah Command "/")
  // Ini mencegah double send karena guard hanya dipanggil di satu pintu ini.
  if (text.startsWith("/")) {
    if (await guardAll(msg)) return;

    // 4. 🎯 LOG CONSOLE (Hanya muncul jika lolos Guard)
    const command = text.split(" ")[0].toLowerCase();
    const username = msg.from.username ? `@${msg.from.username}` : msg.from.first_name;
    const chatType = msg.chat.type === "private" ? "📩 Private" : `👥 Group (${msg.chat.title || "Group"})`;
    const waktuLog = moment().tz("Asia/Jakarta");
    ...
    console.log(
      ...
      chalk.white.bold("\n└ Tanggal : ") + chalk.cyan.bold(`${waktuLog.format("dddd, DD/MM/YYYY")}\n`)
    );
  }
});
...
setInterval(checkPriceMonitor, 60 * 60 * 1000); 
